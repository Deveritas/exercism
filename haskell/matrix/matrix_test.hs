...
matrixTests =
  ...
  , testCase "reshape" $ msInt "1 2\n3 4" @=? reshape (2, 2) (msInt "1 2 3 4")
  , testCase "flatten" $ v [1, 2, 3, 4] @=? flatten (msInt "1 2\n3 4")
  , testCase "matrix of chars" $ fromList ["foo", "bar"] @=? msChar "'f' 'o' 'o'\n'b' 'a' 'r'"
  , testCase "matrix of strings" $ fromList [["this one"], ["may be tricky!"]] @=? msString "\"this one\"\n\"may be tricky!\""
  , testCase "matrix of strings 2" $ fromList [["this one", "one"], ["may be tricky!", "really tricky"]] @=? msString "\"this one\" \"one\" \n\"may be tricky!\" \"really tricky\""
